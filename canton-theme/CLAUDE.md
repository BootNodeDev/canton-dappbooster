...
- The token select's list is a fixed 320px, expressed as `flex: 0 1 320px` with `min-height: 0` and
  never as `height`: a flex item that cannot shrink pushes the card past its own `max-height` on a
  short viewport, and the card has no scroll of its own to catch the spill.
