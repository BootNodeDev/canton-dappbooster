...
- **Bumping `dapp-sdk` means a manual browser pass on the close path.** `guardedConnect` finds the popup by borrowing `window.open`; an SDK that opens its picker any other way degrades it to a bare `sdk.connect()` and silently restores #49. No headless test can catch that — jsdom has no popup to close. Serve `dapp/frontend`, open the picker, close its window, and confirm the button re-enables. Details in [`architecture.md`](architecture.md).
