...
- **Bumping `dapp-sdk` means a manual browser pass on the close path.** No test reaches #49's cause, a real `WindowProxy` losing its `beforeunload`. Serve `dapp/frontend` and close the picker window twice: once on a first connect, once on a connect that reuses a popup the SDK left open (a gateway or WalletConnect wallet). Both must re-enable the button; the second is the one that regressed. Why in [`architecture.md`](architecture.md).
...
- **This package's modules sit at `src/` root, not in kind folders.** `walletAccount`, `connectError`, `connectionMachine`, `connectionActors` and `guardedConnect` are all one flat layer under `src/`; a new one joins them rather than starting a `utils/`. The root rule's kind folders apply to `hooks/`, `testing/` and `mock/`, which exist here.
