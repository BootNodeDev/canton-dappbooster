...
- **Two lifecycle models live here for now — the machine replaces the provider's, in #85.** `connectionMachine.ts` is the connection lifecycle model, internal, unwired, and driven only by its own tests; `CantonConnectProvider.tsx` still owns the state the hooks read until #85 swaps it over (#84 grows the session states first). Put a new lifecycle rule in the machine, not in both, and never "sync" them — a rule living in two places is #76's original disease.
...
- **Bumping `dapp-sdk` means a manual browser pass on the close path.** `guardedConnect` finds the popup by borrowing `window.open`; an SDK that opens its picker any other way degrades it to a bare `sdk.connect()` and silently restores #49. The suite covers the guard's own logic against a stub popup, but not the cause — a real `WindowProxy` losing its `beforeunload` across the `about:blank → blob:` navigation — so no test goes red. Serve `dapp/frontend` and close the picker window twice: once on a first connect, once on a connect that reuses a popup the SDK left open (a gateway or WalletConnect wallet). Both must re-enable the button; the second is the one that regressed. Details in [`architecture.md`](architecture.md).
- **Never import `@canton-network/core-wallet-ui-components`.** It is `dapp-sdk`'s private popup layer, and declaring it to reach `pickWallet` puts a second copy of its module-level popup state in the store, which kills the SDK's retry prompt with `"Wallet picker is not open"`. `guardedConnect.ts` reaches the popup through `window.open` for exactly this reason.
