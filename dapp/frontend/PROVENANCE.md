...
- **Shared kit + theme (new):** added `@bootnodedev/canton-dappbooster` and
  `@bootnodedev/canton-theme`; `main.tsx` imports the theme CSS. The source's own
  party-id truncation and copy helpers were dropped in favour of the kit's
  `<Identifier>` primitive and its `truncateIdentifier` / `partyHint` formatters.
