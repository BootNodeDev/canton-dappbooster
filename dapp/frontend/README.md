...
declines it, claim as it vests, or cancel into a residual claim. Accepting locks the funder's `DBT`
in a `LockedToken` escrow and each claim releases part of it, so the figures on screen are real
