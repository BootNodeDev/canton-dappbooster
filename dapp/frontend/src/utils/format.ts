...
// Amount, grouped, always 2 decimals: rounding drops trailing zeros, and a bare `0` beside
// `2,134.78` reads as a different kind of figure. No unit suffix, which callers add.
...
export const formatTokenFull = (amount: string): string => formatAmount(roundAmount(amount, 10))
...
export const formatTokenCompact = (amount: string): string => {
...
// Whether formatTokenCompact dropped digits, which is the only case where showing the exact figure
